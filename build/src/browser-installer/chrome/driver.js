...
const installChromeDriver = async (chromeVersion, { force = false } = {}) => {
    ...
    const existingLocallyDriverVersion = (0, registry_1.getMatchedDriverVersion)(utils_1.Driver.CHROMEDRIVER, platform, chromeVersion);
    if (existingLocallyDriverVersion && !force) {
        ...
        return (0, registry_1.getBinaryPath)(utils_1.Driver.CHROMEDRIVER, platform, existingLocallyDriverVersion);
    }
    ...
    const buildId = await (0, browsers_1.resolveBuildId)(utils_1.Driver.CHROMEDRIVER, platform, milestone);
    const cacheDir = (0, utils_1.getChromeDriverDir)();
    const canBeInstalled = await (0, browsers_1.canDownload)({ browser: utils_1.Driver.CHROMEDRIVER, platform, buildId, cacheDir });
    if (!canBeInstalled) {
        throw new Error([
    ...
    const installFn = (downloadProgressCallback) => (0, browsers_1.install)({
        ...
        browser: utils_1.Driver.CHROMEDRIVER,
        unpack: true,
        downloadProgressCallback,
    }).then(result => result.executablePath);
    return (0, registry_1.installBinary)(utils_1.Driver.CHROMEDRIVER, platform, buildId, installFn);
};
